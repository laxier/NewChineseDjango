...
# Window settings
WIDTH, HEIGHT = 800, 300  # Increased height to accommodate buttons
...
# Константы
OUTPUT_PREFIX = "my_cut"  # Префикс для названий выходных файлов
...
def plot_waveform():
    ...
    for marker in markers:
        marker_x = int((marker / (len(data) / samplerate)) * WIDTH)
        pygame.draw.line(screen, RED, (marker_x, 0), (marker_x, HEIGHT), 2)

    # Draw current play position in blue
    play_x = int((current_time / 1000 / (len(data) / samplerate)) * WIDTH)
...
def forward_audio(ms):
    global current_time, paused_time
    current_time = min(len(data) / samplerate * 1000, current_time + ms)
    paused_time = current_time  # Update paused position
    pygame.mixer.music.set_pos(current_time / 1000)
...
if __name__ == "__main__":
    # Load audio file
    audio_file = "cut_11.wav"  # Replace with your audio file
